...
namespace ProdajaMobilnihApplikacija.Mobile.ViewModels.Klijent
{
    public class RacunViewModel : BaseViewModel
    {
        ...
        public ObservableCollection<Racun> _RacunList { get; set; } = new ObservableCollection<Racun>();
        ...
        public async Task Init()
        {
            ...
            {
                var KlijentModel = await _klijentService.GetById<ProdajaMobilnihAplikacija.Model.Klijent>(APIService.UserID);
            }


            RacunSearchRequest search = new RacunSearchRequest();

            var list = await _RacunService.Get<List<Racun>>(null);
